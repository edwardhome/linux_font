...
## 🛠️ 核心特性
- **字體視覺大掃除**：暴力移除數百個冗餘系統字體，僅保留開發者專用的微軟核心字體、蘋方 (PingFang) 與 Powerlevel10k 符號字體。
- **輸入法自動化**：整合 **Fcitx5** 體系，配合 Vim 實現 Insert Mode 自動切換語系。
- **韌體開發工具鏈**：一鍵安裝 `gcc`, `make`, `davfs2`, `nfs-common` 等底層開發必備工具。
- **Vim 封神版配置**：自動部署 `vim-plug` 並恢復個人化的 `.vimrc` 配置。
...
```bash
git clone [https://github.com/edwardhome/linux_font.git](https://github.com/edwardhome/linux_font.git)
cd linux_font
chmod +x setup.sh
./setup.sh
